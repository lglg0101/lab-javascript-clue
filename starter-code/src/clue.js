...
{ name: "dumbbell", weight:  0 },
{ name: "poison", weight: 2 },
{ name: "axe", weight:  5 },
{ name: "bat", weight:  3 },
{ name: "trophy", weigh : 25 },
...
three properties, suspect, weapon and room, each holding as a value a card of that 
specific type. 
...
/*Declare a function named revealMystery that receives an envelope object (such as 
the one returned by pickMystery) as the single argument, and returns a reveal message 
in the following format: <FIRST NAME> <LAST NAME> killed Mr. Boddy using the <WEAPON> in the <ROOM>!
*/ 

let string = pickMystery(); 

function revealMystery (envelope) { 
return `${envelope.suspect.firstName} ${envelope.suspect.lastName} killed Mr. Boddy using the ${envelope.weapon.name} in the ${envelope.room.name}!`; 
}; 

revealMystery(string); 
